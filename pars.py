...
@click.command()
@click.option("--track_name", prompt="Track name", help="Write track name")
def tn(track_name):
    url = f"https://ru.hitmotop.com/search?q={track_name}"
...
def download_song(link, title):
    response_song = requests.get(link, timeout=15)
    if os.path.isfile(f"/home/jud/Music/{title}.mp3"):
        return 0
